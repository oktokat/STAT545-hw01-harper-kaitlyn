...
typeof("5")
```
It's a character!! We get this because it's in "quotation marks"...
...
typeof(15.2) # this is a "double"

typeof(typeof(15.2)) # this is a character because it's checking the type of "double"
```
...
dim(iris) #gives you rows and columns
...
summary(iris)[1,] #this gives you the first row of summary, which has all the min values
...
ggplot(iris, aes(Petal.Width)) +
  geom_histogram() +
  labs(title = "Distribution of petal widths", x = "Petal width", y = "Number of flowers")

# make frequency table and make it into a df for better viewing :)
tab1 = data.frame(table(iris$Petal.Width))

rm(tab1, petal_width, iris)
...
# Canada and Algeria
filter(gapminder, country == "Canada" | country == "Algeria") # first way
filter(gapminder, country %in% c("Canada", "Algeria")) # second way

# Canada, Algeria, 1960's
filter(gapminder, country == "Canada" | country == "Algeria", year == 1960:1969) # my way
filter(gapminder, country %in% c("Canada", "Algeria"), year > 1960 & year < 1969) # BETTER way

# all Canada, Algeria only in 60's
filter(gapminder, country == "Canada" | (country == "Algeria" & year == 1960:1969)) # my way
filter(gapminder, country == "Canada" | (country == "Algeria" & year %in% c(1960:1969))) # BETTER way
...
select(gapminder, matches("co")) # pulls out any column with "co" in the name 
```
...
filter(gapminder, gdpPercap > 10000) %>%    #filter all countries with gpd > 10000
  select(-gdpPercap)                        #take all columns EXCEPT gdp column (see dplyr cheatsheet for more tips!)
...
gapminder %>% 
  arrange(year, desc(lifeExp)) %>%         # order df by year, then descending by life expectancy
  select(year, lifeExp, everything())      # put year and life exp at the beginning, but keep all variables
